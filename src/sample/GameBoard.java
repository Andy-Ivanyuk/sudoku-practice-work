...
public class GameBoard {
    ...
    public boolean isCorrect(int x,int y){
        ...
        int temp = combined[x][y];
        for (int i=0;i<9;i++){
            if (i==x)
                continue;
            if (combined[i][y]==temp)
                return false;
        }
        for (int i=0;i<9;i++){
            if (i==y)
                continue;
            if (combined[x][i]==temp)
                return false;
        }
        ...
        for (int i=tempx*3;i<(tempx+1)*3;i++){
            for (int j=tempy*3;j<(tempy+1)*3;j++){
                if (x==i && y==j)
                    continue;
                if (combined[i][j]==temp){
                    return false;
                }
            }
        }
     ...
     */
    public void setNull(int val, int row, int col) {
        player[row][col] = val;
    }
